...
//Explode campo Body e filtra por tabela
val df_hub_explode_body = df_hub
    .select(from_json(col("Body"), schemaHeader).as("values"))
    .select("values.*")
    .filter(col("_id") === lit(s"${tabela}-banqi"))
    .filter(col("destino.quantidade") =!= lit(0))

//Remove '[]' do campo conteudo
val dfLandingTrim = df_hub_explode_body.withColumn("conteudo", trim(col("conteudo"), "[]"))

//Instancia objeto messageHeader
val messageHeader = IngestaoAzure.createHeaderObject(dfLandingTrim)

//Verifica se DF contem dados
messageHeader.isEmptyRaw(dfLandingTrim)

//Cria structType atraves do campo fields
val schema = messageHeader.getConteudoStructType

//Converte todos os campos do schema para string
val string_schema = messageHeader.CreateStringSchema(schema)

//Explode campos do conteudo com schema em String
val dfFormat = dfLandingTrim.withColumn("conteudo", from_json($"conteudo",string_schema))

//Seleciona todos os campos necessarios do conteudo e renomeia os campos anteriores
...
//Retorna campos do conteudo para dataType original apos a explosao
dfClean = messageHeader.castFromString(dfClean, schema)

//Adiciona dados de particao
val dfFinal = dfClean
...
//val query = s"drop table teste_raw_banqi.wallet_spend_money"
//display(spark.sql(query))
//dbutils.fs.rm("""/Felippe/eventhub/raw/banqi/wallet_spend_money""", true)

// COMMAND ----------

val query = s"describe ${messageHeader.getRawTableName}"
display(spark.sql(query))

// COMMAND ----------

val query = s"SELECT * FROM ${messageHeader.getRawTableName}"
display(spark.sql(query))

// COMMAND ----------

val query = s"SELECT COUNT(*), year_partition, month_partition, day_partition FROM ${messageHeader.getRawTableName} GROUP BY year_partition, month_partition, day_partition"
display(spark.sql(query))

// COMMAND ----------

val query = s"SELECT COUNT(*) FROM ${messageHeader.getRawTableName}"
display(spark.sql(query))
...
val query = s"SHOW TABLES FROM teste_raw_banqi"
display(spark.sql(query))

// COMMAND ----------

val query = s"DESCRIBE HISTORY ${messageHeader.getRawTableName}"
display(spark.sql(query))
